...
def prepro_each(traj_path, split, prepro_path, args):
  ...
  if args.add_scene:
    ...
    assert not 0 in scene_oldid2new
  ...
  for video in tqdm(videos, ascii=True):
    ...
    for idx, frame in enumerate(frames):
      ...
      # print("persons_in_cur_seq", persons_in_cur_seq)
  ...
  if args.add_activity:
    ...
    move_cat = [
        utils.activity2id["Walk"],
        utils.activity2id["Run/jog"],
        utils.activity2id["Ride a bike"],
        utils.activity2id["Drive(e.g. a car a truch)"],
        utils.activity2id["Walk with items"],
        utils.activity2id["Follow (a person)"]
    ]
